...
- **Minimum Meteor version**: 3.0.1 is now required
- **Async Query Methods**: Static queries on frontend must use `query.fetchAsync()` instead of `query.fetch(cb)`. `query.fetch()` worked with reactive and static queries before, but now we have to use `Meteor.callAsync()` instead of `Meteor.call()` so underlying `_fetchStatic` method returns a Promise for static queries.
- **Server-side queries**: Must use `fetchAsync`, `fetchOneAsync` and `countAsync` instead of their synchronous counterparts.
