 ...
 *
 * @param {[string, import('meteor/cultofcoders:grapher').Body, import('meteor/cultofcoders:grapher').QueryOptions] | [import('meteor/cultofcoders:grapher').Body, import('meteor/cultofcoders:grapher').QueryOptions]} args
 * @returns {*}
 */
export default (...args) => {
  if (typeof args[0] === 'string') {
    ...
    const entryPointName = Object.keys(body).at(0);
    const collection = Mongo.getCollection(entryPointName);

    if (!collection) {
      throw new Meteor.Error(
        'invalid-name',
        `We could not find any collection with the name "${entryPointName}". Make sure it is imported prior to using this`,
      );
    }
    ...
    if (!collection) {
      if (Meteor.isDevelopment && !NamedQueryStore.get(entryPointName)) {
        console.warn(
          `You are creating a query with the entry point "${entryPointName}", but there was no collection found for it (maybe you forgot to import it client-side?). It's assumed that it's referencing a NamedQuery.`,
        );
      }
 ...
 * @param {import('meteor/cultofcoders:grapher').Body} body
 * @param {import('meteor/cultofcoders:grapher').CreateQueryOptions} options
 * @returns
 */
