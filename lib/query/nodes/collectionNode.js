...
export default class CollectionNode {
    ...
    get collectionNodes() {
        return this.nodes.filter(n => n instanceof CollectionNode)
    }

    get fieldNodes() {
        return this.nodes.filter(n => n instanceof FieldNode);
    }

    get reducerNodes() {
        return this.nodes.filter(n => n instanceof ReducerNode);
    }
     ...
     */
    remove(_node) {
        this.nodes = this.nodes.filter(node => _node !== node);
    }
    ...
    applyFields(filters, options) {
        ...
        for (const n of this.fieldNodes) {
            /**
        ...
        for (const collectionNode of this.collectionNodes) {
        ...
        for (const [field, value] of Object.entries(filters)) {
            ...
            if (!['$or', '$nor', '$not', '$and', '$meta', '$text', '$expr'].includes(field)) {
                // if the field or the parent of the field already exists, don't add it
                if (!has(options.fields, field.split('.')[0])){
                    hasAddedAnyField = true;
    ...
    hasField(fieldName, checkNested = false) {
        ...
        const result = !!this.fieldNodes.find(fieldNode => {
            return options.includes(fieldNode.name);
        });
     ...
     */
    hasReducerNode(name) {
        return !!this.reducerNodes.find(node => {
