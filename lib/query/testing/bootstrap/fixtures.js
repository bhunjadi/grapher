...
import { range, sample, random } from '../../../utils.js';
...
for (const author of authors) {
  ...
  for (const idx of range(POST_PER_USER)) {
    let post = {
      title: `User Post - ${idx}`,
      metadata: {
        keywords: sample(TAGS, random(1, 2)),
        language: {
          ...sample([
    ...
    await postGroupLink.set(sample(groups), { random: Random.id() });

    await postTagsLink.add(sample(tags));

    for (const commentIdx of range(COMMENTS_PER_POST)) {
      let comment = {
        text: sample(COMMENT_TEXT_SAMPLES),
      };

      await postCommentsLink.add(comment);
      await (await Comments.getLink(comment, 'author')).set(sample(authors));
    }
  }
}
