...
export default async function createFixtures() {
  for await (const i of range(CATEGORIES)) {
  ...
  for await (const i of range(GROUPS)) {
  ...
  for await (const groupId of groupIds) {
    for await (const i of range(AUTHOR_PER_GROUPS)) {
      ...
      for await (const j of range(POSTS_PER_AUTHOR)) {
