...
describe("Hypernova", function() {
    ...
    it("Should work with links on nested fields inside nested fields - one", () => {
        const result = Files.createQuery({
            filename: 1,
            meta: {
                type: 1,
        ...
        expect(result).to.be.an('object');
        expect(result.meta).to.be.an('object');
        expect(result.meta.project).to.be.an('object');
        expect(result.meta.project.name).to.be.equal('Project 1');
    });

    it("Should work with links on nested fields inside nested fields (array) - one", () => {
        const result = Files.createQuery({
            filename: 1,
            metas: {
                type: 1,
        ...
        expect(result).to.be.an('object');
        expect(result.metas).to.be.an('array');
        expect(result.metas[0].project).to.be.an('object');
        expect(result.metas[0].project.name).to.be.equal('Project 1');
        expect(result.metas[1].project.name).to.be.equal('Project 2');
    });
    ...
    it("Should work with links on nested fields - one inversed", () => {
        ...
        result.files.forEach(file => {
            ...
            expect(_.keys(file.meta)).to.be.eql(["type", "projectId", "projectIds"]);
        });
    });
    ...
    it("Should work with links on nested fields - many", () => {
        ...
        expect(_.keys(res1.metas[0])).to.be.eql(["type", "projectId", "projectIds"]);
    ...
    it('Should work with links on nested fields inside nested fields - many in object', () => {
        const result = Files.createQuery({
            filename: 1,
            meta: {
                manyProjects: {
        ...
        result.forEach(file => {
            ...
            expect(file.meta.manyProjects.length).to.be.greaterThan(0);
        });
    });

    it('Should work with links on nested fields inside nested fields - many in array', () => {
        const result = Files.createQuery({
            filename: 1,
            metas: {
                manyProjects: {
...
describe("intersectDeep", () => {
    ...
    it('works - with nested fields 4 (nested fields with special fields)', () => {
        ...
        const res = intersectDeep(allowedBody, clientBody);
        // $filter from allowedBody must be preserved
        expect(res.nested).to.be.eql({
            $filter,
            title: 1,
        });
    ...
    it("It should work with reverted link and objects inside array", () => {
        const A = new Mongo.Collection(Random.id());
        const B = new Mongo.Collection(Random.id());
        A.addLinks({
            b: {
...
describe('collection transforms', () => {
    const TransformA = new Mongo.Collection('transformA', { transform: doc => ({ ...doc, hello: 'world1' }) });
    const TransformB = new Mongo.Collection('transformB', { transform:doc => ({ ...doc, hello: 'world2' }) });
    ...
    beforeEach(() => {
        TransformA.remove({});
        TransformB.remove({});
        const b = TransformB.insert({ value: 1 });
        const a1 = TransformA.insert({ value: 1, bLink: b });
        const a2 = TransformA.insert({ value: 2, bLink: b });
    })

    it('Should apply transforms on direct side', () => {
    ...
    it('Should apply transforms on indirect side', () => {
        const [result1, result2] = createQuery({ 
            transformA: {
                value: 1,
                b: { value: 1 }
            }
    ...
    it('Should apply transforms in a nested way', () => {
        const result = createQuery({ 
            transformA: {
                value: 1,
