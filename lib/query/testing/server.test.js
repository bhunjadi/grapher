...
describe('Hypernova', function () {
  ...
  it('Should fetch One links correctly', async function () {
    ...
    data.forEach((comment) => {
      ...
      assert.isTrue(Object.keys(comment.author).length == 2);
    });
  });
  ...
  it('Should fetch One-Inversed links with limit and options', async function () {
    ...
    data.forEach((author) => {
      ...
      author.comments.forEach((comment) => {
  ...
  it('Should fetch Many - inversed links correctly #2', async function () {
    ...
    const tag1Data = data.find((doc) => doc.name === 'Tag1');
    const tag2Data = data.find((doc) => doc.name === 'Tag2');
    const tag3Data = data.find((doc) => doc.name === 'Tag3');

    function hasPost(tag, postName) {
      return !!tag.posts.find((post) => post.name === postName);
    }
  ...
  it('Should fetch One-Meta inversed links correctly', async function () {
    ...
    data.forEach((group) => {
      ...
      assert.lengthOf(Object.keys(group), 3);
  ...
  it('Should fetch Many-Meta links correctly where parent is One link', async function () {
    ...
    data.forEach((post) => {
      ...
      post.author.groups.forEach((group) => {
    ...
    data.forEach((author) => {
      author.posts.forEach((post) => {
        post.comments.forEach((comment) => {
          comment.author.groups.forEach((group) => {
            group.posts.forEach((post) => {
              assert.isObject(post.author);
  ...
  it('Should work with links on nested fields - one inversed', async () => {
    ...
    result.files.forEach((file) => {
      ...
      expect(Object.keys(file.meta)).to.be.eql(['type', 'projectId', 'projectIds']);
    });
  });
  ...
  it('Should work with links on nested fields - many', async () => {
    ...
    expect(Object.keys(res1.metas[0])).to.be.eql([
      'type',
      'projectId',
      'projectIds',
    ]);
    ...
    expect(Object.keys(res2.metas[0])).to.be.eql(['type', 'projectId']);
  ...
  it('Should work with links on nested fields inside nested fields - many in array', async () => {
    ...
    result.forEach((file) => {
      if (file.filename === 'test.txt') {
        ...
        expect(
          file.metas[0].manyProjects.map((p) => ({
            name: p.name,
          })),
        ).to.be.eql([
          {
...
describe('intersectDeep', () => {
  ...
  it('$filters behavior different for many-meta-inversed link', async () => {
    const A = new Mongo.Collection(Random.id());
