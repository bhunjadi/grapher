...
export function createNodes(root) {
  ...
  if (!isObject(root.body)) {
    return;
  }

      Object.entries(root.body).forEach(([fieldName, body]) => {
    if (!body) {
      return;
    }
    ...
    if (linker) {
      ...
      let subroot = new CollectionNode(
...
function isProjectionOperatorExpression(body) {
    ...
    return (
      objKeys.length === 1 &&
      ['$elemMatch', '$meta', '$slice'].includes(objKeys[0])
    );
  }
  return false;
}
...
function tryFindLink(root, dottizedPath) {
  ...
  const potentialLinks = nestedParts.reduce(
    (acc, part) => {
      return [...acc, `${acc.at(-1)}.${part}`];
    },
    [firstPart.join('.')],
  );
...
export function addFieldNode(body, fieldName, root) {
  // it's not a link and not a special variable => we assume it's a field
    ...
    if (!isProjectionOperatorExpression(body)) {
      ...
      Object.entries(dotted).forEach(([key, value]) => {
        // check for link
        ...
        if (linker && !root.hasCollectionNode(linker.linkName)) {
          ...
          const subroot = new CollectionNode(
            linker.getLinkedCollection(),
            subrootBody,
            linker.linkName,
          );
