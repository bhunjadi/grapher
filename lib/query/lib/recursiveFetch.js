...
function fetch(node, parentObject, fetchOptions = {}) {
    ...
    console.log('filters', filters, node.collection._name);

    let results = [];

    if (parentObject) {
        let accessor = node.linker.createLink(parentObject, node.collection._name);
                ...
                if (Array.isArray(value)) {
                    ...
                    value.forEach(item => {
                        ...
                        item[rest.join('.')] = collectionNode.linker.isSingle()
                            ...
                            : collectionNodeResults.filter(result => _.contains(storageValue || [], result._id));
                    });
                }
