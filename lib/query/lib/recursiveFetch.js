...
function fetch(node, parentObject, fetchOptions = {}) {
    ...
    if (fetchOptions.scoped && fetchOptions.subscriptionHandle) {
        Object.assign(filters, fetchOptions.subscriptionHandle.scopeQuery());
    }
    // add query path filter
    if (fetchOptions.scoped) {
        Object.assign(filters, {[`_query_path_${getNodeNamespace(node)}`]: {$exists: true}});
    }
        ...
        for (const result of results) {
            ...
            if (rest.length === 0) {
                ...
                if (Array.isArray(value)) {
                    ...
                    value.forEach(item => {
                        const storageValue = item[nestedPath];
                        // todo: use _id or foreignIdentityField
                        item[rest.join('.')] = collectionNode.linker.isSingle()
                            ? collectionNodeResults.filter(result => result._id === storageValue)
                            : collectionNodeResults.filter(result => (storageValue || []).includes(result._id));
                    });
                }
                else if (isObject(value)) {
                    dot.str(rest.join('.'), collectionNodeResults, value);
                }
            }
...
export default async (node, params, fetchOptions) => {
    node.results = fetch(node, null, fetchOptions);

    await prepareForDelivery(node, params);
