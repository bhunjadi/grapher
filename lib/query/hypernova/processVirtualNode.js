...
function getSlicedResults(results, limit, skip) {
    if (isFiniteNumber(limit) || isFiniteNumber(skip)) {
        skip = skip || 0;
        return results.slice(skip, !isFiniteNumber(limit) ? undefined : skip + limit);
    }
    return results;
}
...
export default function(childCollectionNode, results, metaFilters, options = {}) {
    ...
    if (isMeta && metaFilters) {
        ...
        for (const parentResult of childCollectionNode.parent.results) {
        ...
        for (const parentResult of childCollectionNode.parent.results) {
            ...
            const eligibleResults = results.filter(
                result => {
                    ...
                    return (result[linkStorageField] || []).map(value => value._id).includes(parentResult._id);
                },
            );
        ...
        if (isMany) {
            comparator = (result, parent) => {
                ...
                if (nestedFields.length > 0) {
                    return rootValue.map(nestedObject => dot.pick(nestedFields.join('.'), nestedObject)).includes(parent[linker.foreignIdentityField]);
                }
                return (rootValue ?? []).includes(parent[linker.foreignIdentityField]);
            };
        } else {
            comparator = (result, parent) => dot.pick(linkField, result) == parent[linker.foreignIdentityField];
        }
