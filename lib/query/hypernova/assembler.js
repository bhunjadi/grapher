...
export function assembleMany(parentResult, {
    ...
    if (nestedLinkPath.length > 0 && _.isArray(rootValue)) {
        rootValue.forEach(result => {
            const pickedValue = dot.pick(rest.join('.'), result);
            const normalizedValue = _.isArray(pickedValue) ? pickedValue : (_.isUndefined(pickedValue) ?  [] : [pickedValue]);

            const value = _.flatten(_.union(...normalizedValue).map(id => resultsByKeyId[id]));
