...
export function assembleMany(
  ...
  if (!rootValue) {
    // For foreign identity fields, when the field is missing, leave the link undefined
    if (fieldStorage === linker.foreignIdentityField) {
      return; // Leave undefined
    } else {
      // Materialize empty target when no storage present
      dot.set(childCollectionNode.linkName, [], parentResult);
      return;
    }
  }
  ...
  const data = filterAssembledData(results, { limit, skip });
  dot.set(childCollectionNode.linkName, data, parentResult);
}
...
export function assembleManyMeta(
  ...
  const storages = parentResult[fieldStorage] || [];
  const _ids = Array.isArray(storages) ? pluck(storages, '_id') : [];
  let data = [];
  _ids.forEach((_id) => {
  ...
  dot.set(
    childCollectionNode.linkName,
    filterAssembledData(data, { limit, skip }),
    parentResult,
  );
}
