...
import { groupBy, pluck } from '../../../utils.js';
...
describe('Assembler test', function () {
    ...
    const GROUPED_COMMENTS_BY_ID = groupBy(COMMENTS, '_id');
    const GROUPED_COMMENTS_BY_ORIGINAL_ID = groupBy(COMMENTS, 'originalId');
    ...
    describe('many', function () {
        ...
        describe('nested objects', function () {
            ...
            const NESTED_OBJECT_MANY_LINK_FOREIGN_FIELD_SINGLE_VALUE = new Linker(null, 'meta.comments', {
                type: 'many',
                field: 'meta.commentId',
                foreignIdentityField: 'originalId',
                collection: new Mongo.Collection(null),
            ...
            it('works for nested objects with foreign field - single value', function () {
                const childCollectionNode = {
                    linkName: 'meta.comments',
                };
                ...
                assembleMany(parentResult, {
                    childCollectionNode,
                    linker: NESTED_OBJECT_MANY_LINK_FOREIGN_FIELD_SINGLE_VALUE,
                    resultsByKeyId: GROUPED_COMMENTS_BY_ORIGINAL_ID,
                });
        ...
        describe('nested arrays', function () {
            ...
            const NESTED_ARRAY_MANY_LINK_FOREIGN_FIELD_SINGLE_VALUE = new Linker(null, 'meta.comments', {
                type: 'many',
                field: 'meta.commentId',
                foreignIdentityField: 'originalId',
                collection: new Mongo.Collection(null),
            ...
            it('works for nested arrays', function () {
                ...
                expect(parentResult.meta[0].comments).to.be.an('array').and.have.length(2);
                expect(pluck(parentResult.meta[0].comments, '_id')).to.be.eql([1, 2]);
                expect(parentResult.meta[1].comments).to.be.an('array').and.have.length(1);
            });
            ...
            it('works for nested arrays with foreign field', function () {
                ...
                expect(pluck(parentResult.meta[0].comments, '_id')).to.be.eql([1, 3, 2]);
                expect(parentResult.meta[1].comments).to.be.an('array').and.have.length(1);
            });

            it('works for nested arrays with foreign field - single value', function () {
                const childCollectionNode = {
                    linkName: 'meta.comments',
                };
                ...
                assembleMany(parentResult, {
                    childCollectionNode,
                    linker: NESTED_ARRAY_MANY_LINK_FOREIGN_FIELD_SINGLE_VALUE,
                    resultsByKeyId: GROUPED_COMMENTS_BY_ORIGINAL_ID,
                });

                expect(parentResult.meta[0].comments).to.be.an('array').and.have.length(2);
                expect(pluck(parentResult.meta[0].comments, '_id')).to.be.eql([1, 3]);
                expect(parentResult.meta[1].comments).to.be.an('array').and.have.length(1);
            });
        });
    });
