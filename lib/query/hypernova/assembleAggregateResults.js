    ...
    if (isMeta && isMany) {
        ...
        if (isMany) {
            comparator = (aggregateResult, result) =>
                _.contains(_.isArray(aggregateResult._id) ? aggregateResult._id : [aggregateResult._id], result[linker.foreignField]);
        } else {
            comparator = (aggregateResult, result) =>
                aggregateResult._id == result[linker.foreignField];
        }
