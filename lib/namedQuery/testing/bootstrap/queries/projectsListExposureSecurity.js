...
if (Meteor.isServer) {
  Meteor.methods({
    async updateProject({projectId, ...updates}) {
      await Projects.updateAsync(projectId, {$set: updates});
    },
    async resetProjects() {
      await Projects.updateAsync({name: 'Project 1'}, {$set: {private: false}});
      await Projects.updateAsync({name: 'Project 2'}, {$set: {private: true}});
    }
  })
  ...
  projectsListExposureSecurity.expose({
        ...
        async subscriptionFilter(type, doc, oldDoc, options) {
          ...
          if (dottedPath === '') {
            if (doc.private) {
              ...
              if (!doc.private) {
                ...
                const {projectValue} = await Projects.findOneAsync(doc._id, {fields: {projectValue: 1}});
