...
describe('Named Query', function() {
    ...
    describe.only('Protected reactive queries', () => {
        // beforeEach(done => {
        ...
        it.only('Should work with protected reactive queries after the change', done => {
            const query = projectsListExposureSecurity.clone({});
            const handle = query.subscribe();
                ...
                if (handle.ready()) {
                    ...
                    if (firstCall) {
                        ...
                        Meteor.call('updateProjectToPrivate', {}, (err, res) => {
                            acceptSencondCall = true;
                        });
                    }
                    else if (acceptSencondCall) {
                        c.stop();
                        const projects = query.fetch();
                        handle.stop();
