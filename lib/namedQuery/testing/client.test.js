        ...
        Tracker.autorun(c => {
            if (handle.ready()) {
                ...
                data.forEach(post => {
                    ...
                    const docFromMap = docMap.get(post._id);
                    assert.isObject(docFromMap);
                    assert.equal(docFromMap[scopeField], 1);
                    assert.equal(docFromMap[queryPathField], 1);
                });
    ...
    it('Should work with reactive recursive scoped queries', function (done) {
        ...
        Tracker.autorun(c => {
            if (handle.ready()) {
                ...
                assert.equal(Array.from(docMap.keys()).length, 4);
                ...
                Array.from(docMap.entries()).forEach(([userId, userDoc]) => {
