...
describe('Named Query', function() {
    ...
    describe('Protected reactive queries', () => {
        beforeEach(done => {
            Meteor.call('updateProjectToPublic', {}, done);
        });
        afterEach(done => {
            Meteor.call('updateProjectToPublic', {}, done);
            Tracker.flush();
        ...
        it('Should work with protected reactive queries after the change', done => {
            const query = projectsListExposureSecurity.clone({});
            const handle = query.subscribe();
            ...
            Tracker.autorun(c => {
                if (handle.ready()) {
                    if (firstCall) {
                        ...
                        firstCall = false;
                        Meteor.call('updateProjectToPrivate', {}, (err, res) => {
                        ...
                        if (project1.projectValue === 10000) {
                            // console.log('waiting for potentially one more callback');
                            return;
                        }
        ...
        it('Should work with protected reactive queries also with nested links after the change', function(done) {
