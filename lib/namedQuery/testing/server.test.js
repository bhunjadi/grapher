...
describe('Named Query', function () {
  ...
  it('Should properly cache the values', async function (done) {
    ...
    const Posts = Mongo.getCollection('posts');
  ...
  it('Should work with resolver() that has params server-side validation', async function (done) {
    try {
      await postListParamsCheckServer.clone({}).fetchAsync();
      console.log('done???');
    } catch (e) {
      assert.isObject(e);
      done();
