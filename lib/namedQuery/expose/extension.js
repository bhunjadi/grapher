...
_.extend(NamedQuery.prototype, {
    ...
    _initPublication() {
        ...
        Meteor.publishComposite(this.name, function(params = {}) {
            ...
            if (params.$body) {
                body = intersectDeep(body, params.$body, self.options.intersectOptions);
            }
    ...
    _callFirewall(context, userId, params) {
        ...
        if (Array.isArray(firewall)) {
            for (const fire of firewall) {
                Promise.await(fire.call(context, userId, params));
            }
        } else {
            Promise.await(firewall.call(context, userId, params));
        }
    },
