...
import { prepareForProcess } from '../../main.client.js';
...
export default class Exposure {
  ...
  /**
   *
   * @returns {Grapher.ExposureConfig}
   */
  static getConfig() {
    return globalConfig;
  }
  ...
  /**
   *
   * @param {Grapher.Filters} filters
   * @param {Grapher.Options} options
  ...
  /**
   *
   * @param {Mongo.Collection<unknown>} collection
   * @param {Grapher.ExposureConfig} config
   */
  ...
  initPublication() {
    ...
    publishComposite(this.name, async function ({$body, ...params}) {
      if (!config.blocking) {
        this.unblock();
      }

      const body = await prepareForProcess($body, params);
