...
export function createOne(object, linker) {
    return {
        // Using {$in: []} as a workaround because foreignField which is not _id is not required to be set
        // and {something: undefined} in query returns all the records.
        // $in: [] ensures that nothing will be returned for this query
        [linker.foreignField]: dot.pick(linker.linkStorageField, object) || {$in: []},
    };
}

export function createOneVirtual(object, linker) {
    return {
        [linker.linkStorageField]: object[linker.foreignField] || {$in: []}
    };
}
