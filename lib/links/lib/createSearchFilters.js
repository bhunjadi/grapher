...
import { uniq, union, isObject, pluck } from '../../utils.js';
...
function getIdQueryFieldStorage(object, fieldStorage, isMany = false) {
    ...
    if (Array.isArray(rootValue)) {
        return {$in: uniq(union(...rootValue.map(item => dot.pick(nestedPath, item))))};
    }
...
export function createOneMeta(object, fieldStorage, metaFilters) {
    ...
    if (metaFilters) {
        if (!new Query(metaFilters).test(value)) {
...
export function createManyMeta(object, fieldStorage, metaFilters) {
    ...
    if (metaFilters) {
        value = value.filter(item => new Query(metaFilters).test(item))
    }
