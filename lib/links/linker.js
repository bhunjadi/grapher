...
export default class Linker {
  /**
   * TODO(v3): create static async function because of _initIndex()?
   *
   * @param {Mongo.Collection<unknown>} mainCollection
   * @param {string} linkName
   * @param {Grapher.ProcessedLink} linkConfig
   */
  constructor(mainCollection, linkName, linkConfig) {
    this.mainCollection = mainCollection;
    /** @type {Grapher.ProcessedLink} */
    ...
    if (this.isVirtual()) {
      ...
      // TODO(v3): this is async now and we should await it
      this._initIndex();
    }
  }
  ...
  get foreignIdentityField() {
    if (this.isVirtual()) {
      return (
        this.linkConfig.relatedLinker.linkConfig.foreignIdentityField || '_id'
      );
    }
   ...
   * @returns {Mongo.Collection<unknown>}
   */
  getLinkedCollection() {
  ...
  isSingle() {
    ...
    return this.oneTypes.includes(this.linkConfig.type);
  }
   ...
   * @param {Mongo.Collection<unknown> | null} [collection] To impersonate the getLinkedCollection() of the "Linker"
   *
   * @returns {LinkOne|LinkMany|LinkManyMeta|LinkOneMeta}
   */
  ...
  _validateAndClean() {
    ...
    if (typeof this.linkConfig.collection === 'string') {
      ...
      this.linkConfig.collection = Mongo.getCollection(collectionName);
  ...
  _prepareVirtual() {
    ...
    if (!linker) {
      ...
      Meteor.startup(() => {
        ...
        if (!linker) {
          throw new Meteor.Error(
            `You tried setting up an inversed link in "${this.mainCollection._name}" pointing to collection: "${collection._name}" link: "${inversedBy}", but no such link was found. Maybe a typo ?`,
          );
        } else {
  ...
  _handleReferenceRemovalForVirtualLinks() {
    this.mainCollection.after.remove(async (userId, doc) => {
      ...
      if (!this.linkConfig.relatedLinker) {
        console.warn(
          `There was an error finding the link for removal for collection: "${this.mainCollection._name}" with link: "${this.linkName}". This may occur when you do a .remove() before Meteor.startup()`,
        );
        return;
      }
  ...
  async _initIndex() {
    if (Meteor.isServer) {
      ...
      if (this.linkConfig.index) {
         ...
         */
        const options = this.linkConfig.unique ? { unique: true } : {};
        await this.mainCollection.createIndexAsync({ [field]: 1 }, options);
        ...
        if (this.linkConfig.unique) {
          ...
          // this.mainCollection.createIndex(
          //   {
          //     [field]: 1,
          //   },
          //   options,
  ...
  _initAutoremove() {
    ...
    if (!this.isVirtual()) {
      ...
      this.mainCollection.after.remove(async (userId, doc) => {
        const linker = await this.mainCollection.getLink(doc, this.linkName);
        const docs = await linker.find({}, { fields: { _id: 1 } }).fetchAsync();
   ...
   */
  isDenormalized() {
    return !!(this.linkConfig.denormalize || (this.isVirtual() && this.linkConfig.relatedLinker?.linkConfig?.denormalize));
  }
  ...
  isSubBodyDenormalized(body) {
    const denormBody = this.linkConfig.denormalize?.body || (this.isVirtual() ? this.linkConfig.relatedLinker?.linkConfig?.denormalize?.body : undefined);
    if (!denormBody) return false;

    const cacheBodyFields = Object.keys(dot.dot(denormBody));
    const bodyFields = Object.keys(dot.dot(omit(body, '_id')));

    return (
      bodyFields.length === 0 ||
      bodyFields.every((f) => cacheBodyFields.includes(f))
    );
  }
}
