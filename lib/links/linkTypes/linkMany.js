...
export default class LinkMany extends Link {
  ...
  async add(what) {
    ...
    // update the field
    this.object[field] = union(this.object[field], _ids);
  ...
  async remove(what) {
    ...
    // update the field
    this.object[root] = (this.object[root] || []).filter((_id) => {
      const value = nested.length > 0 ? dot.pick(nested.join('.'), _id) : _id;
      return !_ids.includes(value);
    });
