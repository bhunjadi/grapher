...
export default class LinkManyMeta extends Link {
  ...
  async add(what, metadata = {}) {
    ...
    for (const _id of _ids) {
      let localMetadata = clone(metadata);
  ...
  async metadata(what, extendMetadata) {
    ...
    let existingMetadata = (this.object[field] || []).find(
      (metadata) => metadata._id == _id,
    );
    if (extendMetadata === undefined) {
      return existingMetadata;
    } else {
      Object.assign(existingMetadata, extendMetadata);
  ...
  async remove(what) {
    ...
    this.object[field] = (this.object[field] || []).filter(
      (link) => !(_ids || []).includes(link._id),
    );
